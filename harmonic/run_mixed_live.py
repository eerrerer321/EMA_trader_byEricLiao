...
# 抄底腿「災難保護單」（HARMONIC_DIP_DISASTER_SL，小數如 0.35；預設 0=停用＝現行核准設計）。
# 抄底腿刻意無策略停損（-10% 災難停損回測實證有害），但倉位在交易所端完全裸露：
# 程式掛掉時既無停損也無時間出場。啟用後在進場時掛 entry×(1-X) 的交易所端遠端停損，
# 只作為程式死亡/黑天鵝保險。2026-07-09 MAE 量化（三資料集 48 筆歷史抄底，含 2020-03、
# 2021-05 崩盤）：最深 MAE -30.09%，-35% 保護單歷史零觸發＝零回測成本；
# -30% 會誤傷 2021-05 那筆，勿設更緊。建議值 0.35，是否啟用為 owner 決策。
DIP_DISASTER_SL = float(os.getenv("HARMONIC_DIP_DISASTER_SL", "0"))
# 範圍驗證（對齊 strategy_core 的 TRADE_SYMBOL 啟動即擋）：設 35（想寫 0.35）會算出負停損價、
# 設 1 會算出 0——兩者都讓保險在最需要的崩盤當下靜默失效，必須在啟動時就大聲失敗。
if not (0 <= DIP_DISASTER_SL < 1):
    raise SystemExit(
        f"HARMONIC_DIP_DISASTER_SL 必須是 [0,1) 的小數（0=停用，0.35=進場價-35%），"
        f"目前是 {DIP_DISASTER_SL!r}。")

# 諧波腿開關（HARMONIC_HARM_ENABLED=0 停止產生新諧波訊號；既有掛單/持倉仍管理到自然結束）。
# 2026-07-09 誠實語意重驗（進場根 SL cfda183 之後、資料至 2026-07）：ETH Bybit 實盤場地
# 完整配置回測中，諧波腿 18 筆淨虧 -34 USDT、把 MDD -15.0%→-18.1%、Calmar 1.49→1.25
# （相對 harm off）；實盤實際跑的 1h 諧波獨立回測 PF 1.15、後半段 PF 0.99（edge 已衰減至零）。
# Binance 長史 18-26 諧波仍小幅加分（+125 USDT/37 筆、Calmar 1.46→1.53），故預設維持
# 開啟＝現狀不變；是否停用為 owner 決策（證據見 rolling_backtest_results_cache/mix_sweep_eval.py）。
HARM_ENABLED = os.getenv("HARMONIC_HARM_ENABLED", "1") != "0"
...
class MixedLiveTrader:
    ...
    def _set_exchange_stop_loss(self, stop_price, reason):
        ...
        if not stop_price or stop_price <= 0:
            print(f"⚠️ 停損價無效（{stop_price}），未同步交易所端停損（{reason}）。")
            return False
        ...
        for attempt in range(1, STOP_LOSS_SYNC_RETRIES + 1):
            try:
                ...
                if resp.get("retCode") in (0, "0"):
                    print(f"🛡️ 已同步交易所端停損 ({reason}) @ {params['stopLoss']}")
                    return True
                # Bybit 對「重送相同停損價」回 34040 not modified＝目標值已在交易所上，
                # 冪等重申（S3 趨勢每根重同步、抄底每小時重申）視為成功，不能燒重試又誤報 🚨
                if str(resp.get("retCode")) == "34040" or "not modified" in str(resp.get("retMsg", "")).lower():
                    return True
                print(f"⚠️ 同步交易所端停損失敗 ({attempt}/{STOP_LOSS_SYNC_RETRIES}): {resp}")
            except Exception as e:
                if "34040" in str(e) or "not modified" in str(e).lower():
                    return True  # ccxt 未映射 34040，會以例外拋出——同樣視為冪等成功
    ...
    def _sync_with_exchange(self, reason="同步"):
        ...
        if self.active == "dip_pos" and self.dip is not None:
            self.dip["qty"] = abs(pos_size)  # 抄底腿預設無交易所端停損，僅校正數量
            slp = self._dip_disaster_sl_price()
            if slp:  # 災難保護單啟用時，每次同步重新主張（冪等，對齊趨勢腿模式）
                self._set_exchange_stop_loss(slp, reason)
            return
    ...
    def _open_dip(self, bar):
        ...
        sl = entry * (1 - DIP_DISASTER_SL) if DIP_DISASTER_SL > 0 else None
        sl_desc = f"災難保護單 {sl:.2f}" if sl else "無停損"
        print(f"🟢 [抄底] BTC 3日均費率 {f3*100:+.4f}%/8h 下穿 {DIP_FUNDING_THRESHOLD*100:.2f}% → "
              f"市價做多 {qty} @ ~{entry:.2f}（{DIP_HOLD_BARS} 根/30天時間出場，{sl_desc}）")
        order = self._market("buy", qty, sl=sl)  # 預設無策略 SL（-10% 災難停損實測有害）；啟用時只掛遠端保險
        if order is None:
            return
        # B3：以交易所實際成交均價校正 entry/qty（災難保護單啟用時同步重算）
        entry = self._confirmed_entry(order, "buy", entry)
        filled = self._exch_pos_size()
        if not DRY_RUN and not filled:  # 實單未確認到持倉 → 不建本地部位(避免幻影),暫停待人工確認
        ...
        if filled:
            qty = abs(filled)
        if DIP_DISASTER_SL > 0:
            sl = entry * (1 - DIP_DISASTER_SL)
            self._set_exchange_stop_loss(sl, "抄底災難保護單（成交均價校正）")
        # sl 一併持久化（單一事實來源；None=未啟用）。舊 state 無此鍵時由 _dip_disaster_sl_price 推導。
        self.dip = {"entry": entry, "qty": qty, "entry_time": str(bar.name), "sl": sl}
        self.active = "dip_pos"
        log_event(LOG_FILE, strategy="dip", timeframe=TREND_TF, action="entry", side="long",
                  price=round(entry, 2), qty=qty, sl=(round(sl, 2) if sl else ""), dry=DRY_RUN,
                  order_id=order.get("id", ""), note=f"funding {f3*100:+.4f}%/8h")
        self.save_state()

    def _dip_disaster_sl_price(self):
        """抄底腿災難保護單價（None=未啟用或無持倉）。優先讀持倉時存下的 sl；
        舊 state / 啟用前開的倉無此鍵時由 entry 推導（重啟後保護單自動補掛，語意一致）。"""
        if DIP_DISASTER_SL <= 0 or not self.dip:
            return None
        return self.dip.get("sl") or self.dip["entry"] * (1 - DIP_DISASTER_SL)

    def _manage_dip(self, bar):
        # 災難保護單（啟用時）：實單以交易所端 SL 為主（本檢查是斷線備援）；DRY-RUN 據此模擬觸發。
        sl_px = self._dip_disaster_sl_price()
        if sl_px is not None:
            if float(bar["low"]) <= sl_px:
                print(f"🚨 [抄底] 災難保護單觸發 @ {sl_px:.2f}（進場 {self.dip['entry']:.2f} 的 -{DIP_DISASTER_SL*100:.0f}%）→ 平多")
                ok, fill = self._close_market("sell")
                if not ok:
                    return  # 交易所端 SL 多半已自行觸發（持倉歸零），留給每小時同步對帳
                px = fill or sl_px
                pnl = (px - self.dip["entry"]) * self.dip["qty"] \
                    - self.dip["qty"] * (self.dip["entry"] + px) * FEE
                log_event(LOG_FILE, strategy="dip", timeframe=TREND_TF, action="exit", side="sell",
                          price=round(px, 2), qty=self.dip["qty"], reason="disaster_sl",
                          pnl=round(pnl, 2), dry=DRY_RUN)
                self.dip = None
                self.active = "none"
                self.save_state()
                return
    ...
    def on_1h(self, bar, df):
        ...
        if self.active == "harm_pending":
            ...
            if not HARM_ENABLED:
                return  # 諧波腿停用：不產生新訊號（既有 harm_pending/harm_pos 仍由上方分支管理到結束）
    ...
    def _write_active(self):
        ...
        if self.active == "trend" and self.trend is not None:
            ...
            dip_sl = self._dip_disaster_sl_price()
            items.append({"status": "抄底持倉中", "strategy": "dip", "side": "long", "pattern": "",
                          "entry": round(self.dip["entry"], 2),
                          "stop_loss": round(dip_sl, 2) if dip_sl else "無(設計如此)",
